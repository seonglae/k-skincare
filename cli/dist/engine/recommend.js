...
export function recommend(intake) {
    ...
    const candidates = PRODUCTS.filter((p) => {
        ...
        if (price === undefined)
            return false;
        if (price > limit && p.category !== 'retinoid')
            return false;
    ...
    const citations = intake.concerns.flatMap((c) => CITATIONS[c] ?? []);
    // Warnings
    const warnings = [];
...
function buildRoutine(intake, candidates) {
    const am = [
    ...
    const pmBase = (active) => {
        ...
        if (active)
            steps.push({ step: 2, product: active, note: 'Active layer' });
        steps.push({ step: steps.length + 1, product: 'Moisturizer', note: 'Thick for PM' });
        return steps;
    };
    const pm = {};
    const useRetinoid = intake.concerns.some((c) => ['acne', 'pigmentation', 'wrinkles', 'pores', 'photoaging', 'lentigines'].includes(c));
...
function pickShopping(candidates, intake) {
    ...
    for (const p of candidates) {
        ...
        if (!existing)
            ...
            const score = (q) => q.evidenceFor.filter((c) => intake.concerns.includes(c)).length;
